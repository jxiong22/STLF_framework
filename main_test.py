...
if __name__ == '__main__':
    ...
    model.enc_opt.load_state_dict(checkpoint['encoder_optimizer_state_dict'])
    model.dec_opt.load_state_dict(checkpoint['decoder_optimizer_state_dict'])
    model.fea_opt.load_state_dict(checkpoint['feature_optimizer_state_dict'])
    model.encoder.eval()
    model.decoder.eval()
    model.feature.eval()
    ...
    final_y_pred = predict(model, config, test_Dataloader)

    y_predict = test_Dataloader.dataset.inverse_transform(final_y_pred)
    y_real = test_Dataloader.dataset.targs_ori[config.past_T:]
    _ = evaluate_score(y_real, y_predict)

    logger.info("test end")

    if config.data is "ISONE":
        name = "y_test_ori_ISO.npy"

    else:
        name = "y_test_ori_Utility.npy"

    save(os.path.join(args.dirName, name), y_predict)
    logger.info("-----End-----")
